...
class PersonnelCandidateEvaluation:
    ...
    matched_role_codes: list[str]=field(default_factory=list); warnings: list[str]=field(default_factory=list); explanation: str=""
...
class PersonnelSelectionEngine:
        ...
        for person in self.repository.list_active_personnel(req.tenant_id,req.reference_date):
            ...
            for ra in self.repository.list_role_assignments(personnel_id=person['id'],role_type=role.value,as_of_date=req.reference_date):
                ...
                if not availability['is_available']:
                    ...
                    self.all_evaluations.append(PersonnelCandidateEvaluation(person['id'],person['source_person_key'],person['full_name'],unit['id'],role,0,PersonnelSelectionDecision.NEEDS_CLASSIFICATION,False,[],[ra['role_code']],[warning],'Personnel is unavailable on the reference date.'))
                    continue
                ...
                evaluation=PersonnelCandidateEvaluation(person['id'],person['source_person_key'],person['full_name'],unit['id'],role,score,PersonnelSelectionDecision.SELECTED if score>=90 else PersonnelSelectionDecision.SELECTED_WITH_WARNING,False,[d['domain_code'] for d in matched],[ra['role_code']],warn,f'unit +30; role +25; domain +{level}; availability +10')
                self.all_evaluations.append(evaluation); result.append(evaluation)
        return self.rank_candidates(result)
    def rank_candidates(self,candidates): return sorted(candidates,key=lambda c:(-c.score,c.source_person_key,c.personnel_id))
    def select_for_role(self,role,candidates,count):
        ...
        if role==RuleRoleType.CO_EXECUTOR:
            ...
            chosen=[c for c in candidates if c.score>=75][:count]
            ...
            return PersonnelRoleRecommendation(role,PersonnelSelectionDecision.SELECTED_WITH_WARNING if warnings or chosen[0].score<90 else PersonnelSelectionDecision.SELECTED,selected_personnel_ids=[c.personnel_id for c in chosen],alternative_candidates=candidates[len(chosen):],confidence=min([c.score for c in chosen],default=0),warnings=warnings)
        if len(candidates)>1 and candidates[0].score-candidates[1].score<=PERSONNEL_CONFLICT_DELTA:return PersonnelRoleRecommendation(role,PersonnelSelectionDecision.CONFLICT,alternative_candidates=candidates[:2],warnings=['PERSONNEL_CONFLICT','MULTIPLE_TOP_PERSONNEL'])
    ...
    def resolve_substitute(self,req,unit,role):
        ...
        for row in roots:
            ...
            person=self.repository.get_personnel(row['substitute_personnel_id'])
            if not person or person['tenant_id']!=req.tenant_id or person['status']!='ACTIVE' or self._availability(person['id'],req.reference_date)!='AVAILABLE':
            ...
            for ra in self.repository.list_role_assignments(personnel_id=person['id'],role_type=role.value,as_of_date=req.reference_date):
                ...
                if role in (RuleRoleType.LEAD_EXECUTOR,RuleRoleType.CO_EXECUTOR) and not matched:
                    if graph.get(row['substitute_personnel_id']): self.substitution_warnings.append('SUBSTITUTION_CHAIN_UNSUPPORTED')
                    continue
                seen.add(person['id']); score=min(80,30+25+(10 if ra['is_primary'] else 0)+max([{'PRIMARY':25,'SECONDARY':15,'SUPPORT':8}[d['responsibility_level']] for d in matched],default=0)+10)
                evaluation=PersonnelCandidateEvaluation(person['id'],person['source_person_key'],person['full_name'],unit['id'],role,score,PersonnelSelectionDecision.SELECTED_WITH_WARNING,True,[d['domain_code'] for d in matched],[ra['role_code']],['SUBSTITUTE_USED'],'Direct effective substitute; confidence capped at 80.')
                self.all_evaluations.append(evaluation); result.append(evaluation)
        return self.rank_candidates(result)
