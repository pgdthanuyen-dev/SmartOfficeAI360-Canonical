...
| Login/session | Session-expired flag is set for HTTP authorization failures, login-like HTML, or a login response URL. | Download summary fails acceptance when this flag is set. |
| HTTP/body/integrity | `CATEGORY_n_DOWNLOAD_INTEGRITY_FAILED` when status, size, integrity, or session state fails. | **TEST_VERIFIED**: `test_integrity_validation_accepts_pdf_and_zip`. |
