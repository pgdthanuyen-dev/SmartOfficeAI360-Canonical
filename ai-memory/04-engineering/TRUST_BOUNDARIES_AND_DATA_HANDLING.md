...
| Extracted text → AI proposal | untrusted proposal input | strict JSON, citation/page/excerpt checks | batches/items/warnings | text/proposal metadata | no production provider; raw body not stored | prompt/review operations | CODE_FACT; TEST_VERIFIED |
...
| SmartOffice → Planner | config/remote response untrusted | bounded outcome/idempotency checks | manifest or handoff attempts | credentials/headers/remote id | memory excludes values | no live success/incident guide | CODE_FACT; TEST_VERIFIED |
